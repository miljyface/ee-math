...
class ResultsFormat:
    def __init__(self, norm_type: str = 'none', threshold: float = 99.1, topn: int = 5, precision: int = 20):
        self.norm_type = norm_type
        self.threshold = threshold
        self.num_eigenvalues = topn
        self.num_points = precision

    @classmethod
    def find_convergence_epoch(cls, val_acc_list, threshold=99.0) -> int:
        # Mnist is an easy dataset
    ...
    def train_phase(self) -> dict:
        ...
        model = MNISTNet(norm_type=self.norm_type)
        num_params = sum(p.numel() for p in model.parameters())
        
        print(f"[{self.norm_type.upper()}] Training started")
        ...
        convergence_epoch = ResultsFormat.find_convergence_epoch(history['val_acc'], threshold=99.0)
        ...
        # Save model weights
        weight_path = WEIGHTS_DIR / f"model_{self.norm_type}.pth"
        torch.save(model.state_dict(), weight_path)
        
        # Save training results JSON
        json_path = JSON_DIR / f"training_{self.norm_type}.json"
        with open(json_path, 'w') as f:
            json.dump(results, f, indent=2)
        
        print(f"\n[{self.norm_type.upper()}] Training complete")
        print(f" Final val accuracy: {results['final_metrics']['val_acc']:.2f}%")
        print(f" Max val accuracy: {results['final_metrics']['max_val_acc']:.2f}%")
        print(f" Convergence epoch (99% acc): {convergence_epoch_str}")
    ...
    def analyze_loss_landscape(self, alpha_range=(-0.5, 0.5), beta_range=(-0.5, 0.5)) -> dict:
        # load model
        model = MNISTNet(norm_type=self.norm_type)
        weight_path = WEIGHTS_DIR / f"model_{self.norm_type}.pth"
        ...
        print(f"[{self.norm_type.upper()}] Computing loss landscape")
        ...
        json_path = JSON_DIR / f"landscape_{self.norm_type}.json"
        with open(json_path, 'w') as f:
    ...
    def analyze_hessian(self) -> dict:
        ...
        print(f"[{self.norm_type.upper()}] Computing Hessian eigenvalues")

        start_time = time.time()
        hessian_results = compute_hessian_eigenvalues(
            model, nn.CrossEntropyLoss(), test_loader, device=torch.device("cpu"), k=self.num_eigenvalues, verbose=True
        )
        ...
        json_path = JSON_DIR / f"hessian_{self.norm_type}.json"
        with open(json_path, 'w') as f:
